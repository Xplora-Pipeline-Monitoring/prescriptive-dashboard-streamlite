...
st.set_page_config(page_title="Penjelasan SHAP", layout="wide")
apply_app_theme()
render_page_hero(
    "Penjelasan SHAP untuk risiko Critical",
    "Halaman ini membantu operator memahami fitur utama pendorong risiko dan rekomendasi per node.",
)

_SHAP_MODE_LABEL = {
    "kernel_proxy": "Kernel SHAP (proxy)",
    "fallback_no_shap_pkg": "Fallback (paket SHAP tidak tersedia)",
}

with st.sidebar:
    n_explain = st.slider("Jumlah node untuk dijelaskan", min_value=9, max_value=30, value=18, step=3)
    nsamples = st.slider("Kernel SHAP — jumlah sampel (nsamples)", min_value=40, max_value=120, value=80, step=10)
...
_shap_mode_tampil = _SHAP_MODE_LABEL.get(str(artifacts["shap_mode"]), str(artifacts["shap_mode"]))
st.caption(f"Mode explainability: {_shap_mode_tampil}")
...
mx2.metric("Rata-rata critical_prob", f"{float(per_node_shap_df['critical_prob'].mean()):.3f}" if len(per_node_shap_df) else "0.000")
mx3.metric("Penggerak utama #1", str(all_features_df.iloc[0]["feature"]))

t1, t2 = st.columns(2)
t1.metric("Peringkat temp_avg (SHAP global)", temp_rank_label)
t2.metric("Mean |SHAP| untuk temp_avg", temp_mean_abs_label)

if display_mode == "Top-N":
    st.subheader(f"SHAP global — top-{top_n_features} fitur (mean |SHAP|)")
else:
    st.subheader("SHAP global — semua fitur (mean |SHAP|)")
...
fig_all = px.bar(
    ...
    orientation="h",
    title="Pentingnya fitur global untuk critical_prob",
...
direction_df["arah_dampak"] = direction_df["mean_shap"].apply(
    lambda v: "positif (cenderung menaikkan risiko)"
    if float(v) >= 0
    else "negatif (cenderung menurunkan risiko)"
)
direction_df = direction_df.drop(columns=["mean_shap"])
st.dataframe(direction_df, width="stretch", hide_index=True)

st.subheader("Tabel audit SHAP per node")
cols_priority = [
    "node_id",
    "risk_class",
    "pred_risk_class",
...
c2.metric("Kelas prediksi", str(selected_row.get("pred_risk_class", "N/A")))
c3.metric("critical_prob", f"{float(selected_row.get('critical_prob', 0.0)):.4f}")
c4.metric("Audit", str(selected_row.get("audit_flag", "N/A")))

st.markdown("**3 penggerak SHAP teratas**")
...
if shap_values_matrix is not None and feature_columns and explain_nodes:
    ...
    st.subheader("Semua nilai SHAP per node")
